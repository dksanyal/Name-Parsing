...
import argparse
from word_seq2seq_utils import *
...
if __name__ == '__main__':
    ...
    X, X_vocab_len, X_word_to_ix, X_ix_to_word, y, y_vocab_len, y_word_to_ix, y_ix_to_word = load_data('train.names', 'train.states', MAX_LEN, VOCAB_SIZE)
    ...
    # Performing test if we chose test mode
        ...
        if len(saved_weights) == 0:
            ...
            X_test_without_pad = load_test_data('test.names', X_word_to_ix, MAX_LEN)
            X_test = pad_sequences(X_test_without_pad, maxlen=X_max_len, dtype='int32')
            model.load_weights(saved_weights)
            predictions = np.argmax(model.predict(X_test), axis=2)
            sequences = []
            for i in range(len(predictions)):
                sequence = ' '.join([y_ix_to_word[predictions[i][index]] for index in range(X_max_len-len(X_test_without_pad[i]),len(predictions[i])) if index > 0])
                sequences.append(sequence)
            np.savetxt('testresult', sequences, fmt='%s')
